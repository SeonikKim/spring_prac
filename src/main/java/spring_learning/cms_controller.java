...
public class cms_controller {
	...
	public String cmsview(Model m ) {
		//데이터 그룹 한개만 가져옴
		/*
		 mapper에서 resultType을 String으로 처리했을 경우는 하나의 컬럼값에 대해서만 처리할 때 사용할 수 있음
		 단, 여러개의 컬럼으로 처리 시 첫번째 컬럼 외 모두 loss됨. (ex : count, avg, sum, min, max ... 사용) 
		 */
...
//		System.out.println(result.get(0).get("cate"));
	...
	public String cmsok(@RequestParam String csubject, @RequestParam String cuser,
			@RequestParam(name = "cate", required = false) ArrayList<String> cate) throws Exception {
		/*ArrayList<String> 클래스 배열로 동일한 checkbox 처리함
		 checkbox가 동일한 이름에 여러개가 있을 경우 배열로 값을 받으며, DB에 저장 시 String으로 변환하여 String.join()클래스를 이용하여
		DB set이라는 자료형으로 저장
		*/
		...
		//mapper.xml에 다른 테이블을 사용하더라도 문제는 X
		//단점 : 유지보수 시 해당 쿼리문을 찾아야함
